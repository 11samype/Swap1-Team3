...
public class WorkerSetup extends javax.swing.JFrame {
	...
	// 1. Extract Method was used here to refactor the code in addWorker as
	// suggested so
	// that everthing wasn't clumped into one method and responsibilities were
	// split up into 3 smaller methods.
	// 2. If the way adding a worker changes, or the
	// way the layout for them is setup needs to change, this can be done from
	// within their own methods.
	// 3. While the refactoring was successful in making addWorker a smaller
	// method, some of the methods extracted I feel still aren't small enough
	// and could use some work so that there is not a lot of parameter passing
	// between the methods extracted and duplicate code. However the purpose of
	// making this method smaller was achieved.
	...
	private void addWorker() {
		...
		workerDayTabSetup(tempWorkerDays);
		addWorkerNameAndTextArea(tempWorkerDays, tempWorkerName, tempWorkerTab);

	}


	private void workerDayTabSetup(javax.swing.JTabbedPane tempWorkerDays){
		this.days = ConfigManager.getDays();
	...
	private void addWorkerNameAndTextArea(JTabbedPane tempWorkerDays, JTextField tempWorkerName, JPanel tempWorkerTab){
								...
								.addGroup(
										workerTab1Layout
												...
												.addContainerGap(
														javax.swing.GroupLayout.DEFAULT_SIZE,
														Short.MAX_VALUE)));
				checkForNullPointerInTabs(tempWorkerTab);
	}
	
	private void checkForNullPointerInTabs(JPanel tempWorkerTab){
	...
	private void nextButtonActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (allGood) {
			HTMLGenerator.reset();
			ConfigManager.setWorkers(workers);
			ConfigManager.setSchedule(new Schedule(ConfigManager.getDays(), ConfigManager.getWorkers()));
			ConfigManager.dumpConfigFile();
			ConfigManager.cal = new CalendarGUI(ConfigManager.getSchedule(), ConfigManager.getLocale(), ConfigManager.getColor());
